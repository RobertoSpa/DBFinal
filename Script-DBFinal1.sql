...
-- Nota Fiscal Pedido º 1

select p.ped_cd_id as id, cli_tx_nome as "Nome do Cliente", ped_dt_data_do_pedido, prod_dec_valor_unitario, prod_tx_nome
from pedido as p 
join cliente as c
on (p.cli_cd_id = c.cli_cd_id )
join "itemPedido" ip 
on (ip.ped_cd_id = p.ped_cd_id)
join produto as prod 
on (prod.prod_cd_id = ip.prod_cd_id);

-- somar os valores por group by iped_cd_id
